...
interface ListViewWithRealtimeProps {
  projectId: string;
}

export function ListViewWithRealtime({
  projectId
}: ListViewWithRealtimeProps) {
  ...
  useEffect(() => {
    const viewers = onlineUsers
      .filter((user) => user.currentView?.entityId === projectId && user.currentView?.viewType === "list")
      .map((user) => ({
        id: user.userId,
        name: user.userName,
